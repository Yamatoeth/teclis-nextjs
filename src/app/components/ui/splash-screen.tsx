...
export default function SplashScreen({ onEnter, title, subtitle }: SplashScreenProps) {
  ...
  useEffect(() => {
    ...
    // Set initial states
    gsap.set([brandRef.current, titleRef.current, subtitleRef.current, buttonRef.current], { opacity: 0, y: 50 });
    gsap.set([leftPanelRef.current, rightPanelRef.current], { x: 0 });

    // Initial animation - brand, title and button fade in
  ...
  const handleEnter = () => {
    ...
    // Fade out brand, title, subtitle and button
    const elementsToFade = [brandRef.current, titleRef.current, buttonRef.current];
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-white z-20">
        ...
        <p 
          ref={brandRef}
          className="text-sm md:text-base text-emerald-400/80 tracking-widest mb-6 uppercase font-semibold"
        >
          teclis scientific
        </p>
